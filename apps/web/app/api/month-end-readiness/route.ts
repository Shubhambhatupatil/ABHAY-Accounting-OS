...
export async function GET(request: NextRequest) {
  return proxyAiEntryGet(request, "month-end-readiness");
}

async function proxyAiEntryGet(request: NextRequest, endpoint: string) {
  ...
  try {
    const response = await fetch(`${apiUrl}/companies/${companyId}/ai-entry/${endpoint}`, {
      method: "GET",
      cache: "no-store",
      headers: {
