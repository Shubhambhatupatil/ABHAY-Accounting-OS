...
try {
  if (isSignup) {
    const result = await supabase.auth.signUp({
      ...
      options: {
        data: {
          ...
          initial_company_name: companyName.trim()
        }
      }
    });
     ...
     * session may be null until user verifies email.
     */
    if (!accessToken) {
    ...
    if (companyName.trim()) {
      try {
        await accountingApi.createCompany(
          accessToken,
          {
...
try {
  ...
  if (!token) {
    throw new Error(
      "Alpha Demo Mode is not enabled for this deployment."
    );
  }
  ...
  setError(
    caught instanceof Error
      ? caught.message
      : "Alpha Demo Mode is unavailable."
  );
} finally {
  setIsSubmitting(false);
}
...
return ( <form
 ...
 > <div className="mb-5"> <span className="ai-badge mb-3">
AI Accounting Alpha </span>
...
```
  ...
  {alphaDemoMode ? (
    <p className="mt-3 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs leading-5 text-amber-800">
      Alpha Demo Mode is enabled for this live
      demo. Real Supabase login remains available.
    </p>
  ) : null}
